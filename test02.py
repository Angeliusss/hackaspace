...
def comparer():
    """
    Récupère les dictionnaires sélectionnés à partir des combobox,
    crée un fichier JSON avec les
    dictionnaires sélectionnés, sauvegarde les valeurs de seuil et
    pas dans un fichier texte, ferme
    l'application courante et exécute un autre script.
    """
...
def read_file_and_update_labels():
    """
    Ouvre le fichier contenant les noms des fichiers sélectionnés, lit les
    noms des fichiers, ouvre
    ces fichiers, les lit, ajoute les données lues à une liste,
    met à jour le texte des labels et 
    affiche les données lues dans la console.
    """
...
def update_text_area(event):
    """
    Met à jour le contenu d'un widget Text en fonction de la sélection
    dans la combobox. Récupère
    le numéro du candidat sélectionné, récupère le dictionnaire du candidat,
    crée une chaîne à afficher
    et met à jour le widget Text avec cette chaîne.
    """
    ...
    # Soustraire 1 car l'indexation commence à 0
    num_candidat = int(selection.split(" ")[-1]) - 1
...
def update_text_area_B(event):
    """
    Met à jour le contenu d'un autre widget Text en fonction de la sélection
    dans la deuxième combobox.
    Récupère le numéro du candidat sélectionné, récupère le dictionnaire
    du candidat, crée une chaîne
    à afficher et met à jour le widget Text avec cette chaîne.
    """
    ...
    text_area_candidats_fichier_B.delete(1.0, tk.END)

    # Insérer le nouveau contenu
    text_area_candidats_fichier_B.insert(tk.END, text_to_display)
...
# Crée un champ d'entrée
entry_def_seuil = tk.Entry(app)

# Affiche le champ d'entrée dans l'interface utilisateur
entry_def_seuil.pack()
